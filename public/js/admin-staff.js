...
async function loadAdminStaff() {
  ...
  container.innerHTML = `
    <div class="mb-6">
      ...
      <p class="text-gray-600">스태프 계정 승인 및 권한 관리</p>
    </div>
...
function renderPendingUsers(users) {
  ...
  if (users.length === 0) {
    container.innerHTML = `
      <div class="text-center py-8 text-gray-500">승인 대기 중인 사용자가 없습니다</div>
    `;
    return;
  }
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          ${staff.map(user => `
            <tr class="hover:bg-gray-50">
              ...
              <td class="px-4 py-3 text-center">
                ${user.status === 'ACTIVE' ? `
                  <button onclick="revokeStaff('${user.user_id}', '${user.email}')"
...
async function revokeStaff(userId, email) {
  if (!confirm(`${email}의 권한을 철회하시겠습니까?`)) {
    return;
  }

  try {
    await apiCall('/admin/revoke-staff', {
      method: 'POST',
